<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $sql = "select * from (select s.*, GROUP_CONCAT(es.employee) as employees from shift s left join employeeshift es on s.id = es.shift group by s.id) employee 
                where employee.scheduledDate between '".$_POST["from"]."' and '".$_POST["till"]."' order by employee.scheduledDate";
