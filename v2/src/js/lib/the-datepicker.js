    ...
    var Datepicker = (function () {
        function Datepicker(input, container, options) {
            ...
            if (!container) {
                ...
                if (input) {
                    var CheckIfAttached = setInterval(() => {
                        if (!isEmpty(input.parentNode)) {
                            input.parentNode.appendChild(container);
                            window.clearInterval(CheckIfAttached);
                        }
                    }, 1000);
                }
            } else {
...
(function (TheDatepicker) {
    var HtmlHelper_ = (function () {
        ...
        HtmlHelper_.prototype.createSelectInput_ = function (options, _onChange) {
            var refactoredOptions = {};
            Object.entries(options).forEach(entry => {
                refactoredOptions[entry[0]] = entry[1]["label"];
            });

            var dropdown = new Dropdown({
                data: refactoredOptions,
                placeholder: Object.keys(refactoredOptions).length > 0 ? "z.B. " + Object.values(refactoredOptions)[0] : ""
            }).metaname("datepickerSelect").isSmallSize(true).changeListener((event) => {
                _onChange(event, dropdown.getValue());
                // TODO Slide Funktion wäre hier optisch passend, wenn sich das verknüpfen lässt 
...
(function (TheDatepicker) {
    var Template_ = (function () {
        ...
        Template_.prototype.updateMonthElement_ = function (viewModel) {
            ...
            this.monthElement_.style.display = 'none';
            this.monthSelect_.setValue(currentMonth + '');
        };
        Template_.prototype.createYearElement_ = function (viewModel) {
            ...
            for (var index = diff.prepend.length - 1; index >= 0; index--) {
                options[diff.prepend[index]] = {
                    label: diff.prepend[index]
                };
            }

            var selectElement = this.htmlHelper_.createSelectInput_(options, function (event, year) {
                var currentMonth = viewModel.getCurrentMonth_();
                var newMonth = new Date(currentMonth.getTime());
                ...
                if (!viewModel.goToMonth_(event, newMonth)) {
                    // _this.yearSelect_.setValue(currentMonth.getFullYear() + '');
                }
            });

            var yearElement = this.htmlHelper_.createDiv_('year');
            var yearContent = this.htmlHelper_.createSpan_();
            yearElement.appendChild(selectElement.build());
            ...
            this.yearElement_.style.display = 'none';
        ...
        Template_.prototype.updateYearElement_ = function (viewModel) {
            ...
            var currentYear = viewModel.getCurrentMonth_().getFullYear();
            this.yearSelect_.setValue(currentYear + '');
        };
        Template_.prototype.createMonthAndYearElement_ = function (viewModel) {
            ...
            monthAndYear.appendChild(selectElement.build());
            return monthAndYear;
        };
...
(function (TheDatepicker) {
    ...
    var Translator = (function () {
        function Translator() {
            ...
            this.titles_ = (_a = {},
                _a[TitleName.GoBack] = 'Vorheriger Monat',
                _a[TitleName.GoForward] = 'Nächster Monat',
                _a[TitleName.Close] = 'Schließen',
                _a[TitleName.Reset] = 'Löschen',
                _a[TitleName.Deselect] = 'Löschen',
                _a);
        }
